...
const safeText = (v) => loader.normalizeText(v);
const safeArray = (v) => loader.normalizeList(v);
const safeGet = (obj, path, fallback = "") => {
    const val = path.split('.').reduce((acc, part) => (acc && acc[part] !== undefined) ? acc[part] : undefined, obj);
    if (val === undefined || val === null) return fallback;
    if (typeof val === 'string' && (val.toLowerCase() === 'undefined' || val.toLowerCase() === 'null')) return fallback;
    return val;
};
