    ...
    try {
        ...
        container.innerHTML = `
            <div class="animate-in fade-in slide-in-from-bottom-4 duration-700">
                ...
                <!-- TODAY STRATEGY Banner (STEP-46 & STEP-48) -->
                <div class="mb-8 bg-gradient-to-r from-slate-900 to-indigo-900/30 border border-indigo-500/30 rounded-2xl overflow-hidden shadow-2xl">
                    <div class="p-6 flex flex-wrap items-center justify-between gap-6">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <div class="text-lg font-black text-white">Theme: <span class="text-indigo-300 font-extrabold">${brief.core_theme}</span></div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-8">
                            <div class="text-center">
                                ...
                                <div class="text-sm font-black ${getActionColorClass(brief.investment_decision?.action)}">${brief.investment_decision?.action || 'WATCH'}</div>
                            </div>
                            <div class="text-center">
                                <div class="text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1">Timing</div>
                                <div class="text-sm font-black text-white">${brief.investment_decision?.timing || 'UNKNOWN'}</div>
                            </div>
                            <div class="text-center">
                                <div class="text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1">Risk Level</div>
                                <div class="text-sm font-black ${getRiskColorClass(risk.risk_level)}">${risk.risk_level}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="px-6 py-2.5 bg-black/40 border-t border-indigo-500/10 flex items-center justify-between">
                         <div class="flex items-center gap-2">
                            ...
                            <span class="text-[10px] font-bold text-slate-400">${risk.invalidation}</span>
                         </div>
                         <div class="text-[9px] font-black text-indigo-400">SCORE: ${risk.risk_score.toFixed(2)}</div>
                    </div>

                    <!-- TODAY PORTFOLIO (NEW STEP-50) -->
                    <div class="px-6 py-4 bg-slate-900/40 border-t border-indigo-500/10 grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div class="md:col-span-3 flex flex-wrap gap-3">
                            ${(brief.portfolio_allocation?.allocations || []).filter(a => a.weight > 0).map(a => `
                                <div class="px-3 py-1.5 bg-[#0d131f] border border-indigo-500/20 rounded-lg flex items-center gap-3">
                                    <span class="text-[10px] font-black text-blue-400">${a.ticker}</span>
                                    <span class="text-[10px] font-black text-white">${(a.weight * 100).toFixed(1)}%</span>
                                </div>
