...
def build_operator_view(project_root: Path):
    ...
    Source: data/operator/today_operator_brief.json (REAL SSOT)
    Output: docs/data/ui/ui_operator_view.json (UI CONTRACT)
    """
    # The actual engine output location for consistency
    ...
    if not brief_path.exists():
        print(f"[STEP-L] ⚠️ Genuine SSOT missing at {brief_path}. Trying fallback...")
        brief_path = project_root / "docs" / "data" / "ops" / "today_operator_brief.json"
        
    if not brief_path.exists():
        print(f"[STEP-L] ❌ NO SSOT found. Aborting.")
        return None
        
    brief = json.loads(brief_path.read_text(encoding="utf-8"))

    # Extract Data (Mapping from AI Power Constraint schema)
    ui_today = brief.get("ui_today", {})
    inv_dec = brief.get("investment_decision", {})
    
    today_topic = brief.get("core_theme", ui_today.get("title", "N/A"))
    why_now = ui_today.get("why_now", brief.get("narrative", {}).get("explanation", "시장 분석 완료."))
