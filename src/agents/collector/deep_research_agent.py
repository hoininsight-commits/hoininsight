...
class DeepResearchAgent:
    ...
    def generate_queries(self, raw_context: str) -> List[str]:
        ...
        그런 다음, 해당 이벤트가 **한국 증시(K-Stock)**의 특정 종목, 독점 기술, 납품 이력 등과 어떻게 연결될지 심층 분석하기 위한 **검색어 4개**를 생성하세요.

        [오늘의 데이터]
        {raw_context[:3000]}

        [검색어 작성 가이드]
        - **검색어 1~3**: 한국 수혜주·구조 탐색 쿼리
          ...
          - 예: "반도체 냉각 액침 수냉 데이터센터 수혜주 실적", "HBM 공급 부족 2026 삼성 하이닉스 점유율"

        - **검색어 4 (IB 리포트 탐색 — 반드시 포함)**:
          - 오늘 핵심 이벤트 키워드 + "analyst report target price raised 2026" 또는 "Goldman Sachs UBS JP Morgan 목표주가 상향" 형식으로 생성하세요.
          - IB 리포트의 구체적 수치(목표주가, 등급 변경, EPS 상향)가 드러나는 검색어여야 합니다.
          - 예: "UBS Samsung semiconductor HBM target price raised 2026", "Goldman Sachs Korea KOSPI overweight report 2026"

        반드시 아래 JSON 형식으로만 반환하세요:
        {{
            "target_event": "선정된 글로벌 이벤트",
            "queries": ["한국수혜주 탐색 쿼리1", "한국수혜주 탐색 쿼리2", "한국수혜주 탐색 쿼리3", "IB 리포트 탐색 쿼리4"]
        }}
        """
