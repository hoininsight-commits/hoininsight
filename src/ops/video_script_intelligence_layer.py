...
class VideoScriptIntelligenceLayer:
    ...
    def _generate_script(self, candidate: Dict, narrative_topics: List[Dict], decision_card: Dict) -> Dict:
        """
        Generates a structured script based on candidate and engine context.
        """
        ds_id = candidate.get("dataset_id")
        title = candidate.get("title", "Unknown Topic")
        
        # 1. Extract matching narrative content
        narrative = next((t for t in narrative_topics if t.get("dataset_id") == ds_id), {})
        
        # 2. Extract decision/causal context
        causal = narrative.get("causal_chain", {})
        if not causal:
            # Fallback to decision card if direct link is missing
        ...
        # 6. Mentionables (Placeholder/Logic from Theme Stocks)
        # In a real scenario, this would come from a ticker-mapping service
        mentionables = []
