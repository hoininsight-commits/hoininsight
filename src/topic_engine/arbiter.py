...
class TopicArbiter:
 ...
 ### 🏹 [사냥꾼의 사냥 지침: 글로벌 원인론]
 1. **글로벌 변곡점 사냥 (Global Trigger)**: 오늘 한국 뉴스에서 시끄러운 주제가 아니라, **어제 미국/글로벌 시장에서 발생한 '이상 현상(폭등/폭락/기술 표준 변경)'**을 먼저 사냥하십시오. 
 2. **K-평행이론 매칭 (Parallelism)**: 글로벌 시장을 뒤흔든 그 '원인'이 한국 증시의 특정 섹터나 종목(공급망의 핵심, 기술적 복제 모델)으로 어떻게 전이될지 인과관계를 증명하십시오. (예: "WDC가 올랐으니 한국의 기판주 TLB가 오른다")
 3. **DART로 최종 사냥 (The Kill)**: 글로벌 파동이 도달할 길목에 서 있는 한국 기업 중, **DART 공시(수주, 시설투자)**나 **구체적인 수치(수출 데이터)**로 그 실체가 증명된 종목을 최종 사냥 대상으로 선정하십시오.

 **[선정 기준]**
 1. **글로벌 동조화 (Global Alignment)**: 단순히 국내에서만 핫한 테마는 무시하십시오. 반드시 글로벌 거대 자본의 움직임과 궤를 같이해야 합니다.
 2. **기술적/물리적 필연성**: "왜 이 변화가 거부할 수 없는 흐름인가?"를 기술적 병목(Bottleneck)이나 정책적 강제성으로 증명하십시오.
 3. **내일의 주인공 (Next Money Spot)**: 이미 상한가를 친 종목이 아니라, 글로벌 파동이 내일 아침 한국 개장과 동시에 쏟아질 '길목'을 선점하십시오.
