...
class TopicArbiter:
 ...
 ### 🇰🇷 [국내 시장 사냥 및 선정 지침]
 제공된 데이터 중 특히 **국내 경제 뉴스(hankyung, mk, yna)**와 **DART(공시) 데이터**를 최우선으로 분석하십시오.

 **[사냥꾼의 K-로직]**
 1.  **DART 기반 실전 사냥**: 국내 상장사의 '단일판매 공급계약', '신규 시설투자', '제3자 배정 유상증자' 등 실제 돈이 움직이는 공시를 사냥의 핵심 팩트로 삼아라.
 2.  **글로벌 파도 -> 한국 상륙**: 미국 테크주(엔비디아, 테슬라 등)의 변동이나 글로벌 유가/금리 변화가 한국의 특정 섹터(HBM, 방산, 조선, 원전, K-푸드 등)에 어떤 '직격탄' 혹은 '호재'가 되는지 연결해라.
 3.  **K-수급과 테마의 배신**: 지수는 빠지는데 특정 테마로 수급이 쏠리는 '돈의 냄새'를 맡아라.

 **[선정 기준]**
 1.  **국내 종목 연결성 (K-Stock Impact)**: 반드시 한국 상장사나 국내 테마주와 연결되는 이슈인가?
 2.  **DART 확증 (Hard Evidence)**: 카더라 뉴스가 아니라 DART 공시나 구체적인 수출 데이터(ECOS)로 증명 가능한 기회인가?
 3.  **내일의 주인공 (Next Theme)**: 지금 당장 불붙은 테마 말고, 내일 아침 개장과 동시에 돈이 쏠릴 수밖에 없는 논리적 길목인가?
