...
def _run_ecos_collector(key: str, base_dir: Path, category: str, name: str) -> Path:
    collector = ECOSCollector()
    success = collector.collect_series(key)
    if not success:
        print(f"[ECOS] Failed to collect {name}")
        return None
    return _get_latest_ecos_path(category, name)

def write_raw_base_rate(base_dir: Path) -> Path:
    """Collect Korea Base Rate"""
    return _run_ecos_collector('722Y001/0101000', base_dir, 'rates', 'korea_base_rate')

def write_raw_cpi(base_dir: Path) -> Path:
    """Collect Korea CPI"""
    return _run_ecos_collector('901Y009/0', base_dir, 'inflation', 'korea_cpi')

def write_raw_usdkrw(base_dir: Path) -> Path:
    """Collect USD/KRW Exchange Rate"""
    return _run_ecos_collector('731Y001/0000001', base_dir, 'fx', 'korea_usdkrw')
